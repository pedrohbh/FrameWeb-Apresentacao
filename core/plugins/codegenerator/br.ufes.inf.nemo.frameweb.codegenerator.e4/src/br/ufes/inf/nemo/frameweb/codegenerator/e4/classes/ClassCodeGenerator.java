...
public class ClassCodeGenerator
{
	...
	public static String render(DomainClass class_, String template)
	{
		...
		templateEngineContext.addParameter(PACKAGE, class_.getPackage()).addParameter(CLASS, class_)
				.addParameter(ATTRIBUTES, class_.getAttributes())
				.addParameter(ASSOCIATIONS,
						class_.getAssociations().stream().filter(DomainAssociation.class::isInstance)
								.map(DomainAssociation.class::cast).collect(Collectors.toList()))
				.addParameter(METHODS,
						class_.getOperations().stream().filter(DomainMethod.class::isInstance)
	...
	public static String render(Enumeration class_, String template)
	{
		...
		templateEngineContext.addParameter(PACKAGE, class_.getPackage()).addParameter(CLASS, class_).addParameter(
				LITERALS,
				class_.getOwnedLiterals().stream().map(EnumerationLiteral::getName).collect(Collectors.toList()));
	...
	public static String render(FrontControllerClass class_, String template)
	{
		...
		templateEngineContext.addParameter(PACKAGE, class_.getPackage()).addParameter(CLASS, class_)
				.addParameter(ATTRIBUTES, class_.getAttributes())
				.addParameter(ASSOCIATIONS,
						class_.getAssociations().stream().filter(ServiceControllerAssociation.class::isInstance)
								.map(ServiceControllerAssociation.class::cast).collect(Collectors.toList()))
				.addParameter(METHODS,
						class_.getOperations().stream().filter(FrontControllerMethod.class::isInstance)
...
//	TODO terminar a implementacao de REST para o FrameWeb
	public static String render(RestControllerClass class_, String template)
	{
		...
		templateEngineContext.addParameter(PACKAGE, class_.getPackage()).addParameter(CLASS, class_)
				.addParameter(ATTRIBUTES, class_.getAttributes()).addParameter(ASSOCIATIONS, class_.getAssociations())
				.addParameter(METHODS,
						class_.getOperations().stream().filter(RestControllerMethod.class::isInstance)
								.map(RestControllerMethod.class::cast).collect(Collectors.toList()))
				.addParameter(GENERALIZATIONS, class_.getGeneralizations());
			...
			Map<String, Object> partialProperties, List<ReferencedPartials> partiaisReferenciados, String template)
	{
		...
		templateEngineContext.addParameter(PARTIAL, partial)
				...
				.addParameter(PARTIAL_PROPERTIES, partialProperties);
...
//	TODO implementar o renderizador de pagina html
	public static String render(Page page, List<UIComponent> pageUIComponents, String template)
	{
		...
		templateEngineContext.addParameter(PAGE, page)
				.addParameter(NAVIGATION_ASSOCIATIONS,
						page.getAssociations().stream().filter(NavigationAssociation.class::isInstance)
								.map(NavigationAssociation.class::cast).collect(Collectors.toList()))
...
//		fazendo com que seja necessario resgatar os metodos direto da classe que implementa a interface
		templateEngineContext.addParameter(PACKAGE, interface_.getPackage()).addParameter(INTERFACE, interface_)
	...
	public static String render(DAOClass class_, String template)
	{
		...
		templateEngineContext.addParameter(PACKAGE, class_.getPackage()).addParameter(CLASS, class_)
				.addParameter(ATTRIBUTES, class_.getAttributes()).addParameter(METHODS, class_.getOperations().stream()
						.filter(DAOMethod.class::isInstance).map(DAOMethod.class::cast).collect(Collectors.toList()))
...
//		fazendo com que seja necessario resgatar os metodos direto da classe que implementa a interface
	...
	public static String render(ServiceClass class_, String template)
	{
		...
		templateEngineContext.addParameter(PACKAGE, class_.getPackage()).addParameter(CLASS, class_)
				.addParameter(ATTRIBUTES, class_.getAttributes())
				.addParameter(ASSOCIATIONS,
						class_.getAssociations().stream().filter(DAOServiceAssociation.class::isInstance)
								.map(DAOServiceAssociation.class::cast).collect(Collectors.toList()))
				.addParameter(METHODS, class_.getOperations().stream().filter(ServiceMethod.class::isInstance)
						.map(ServiceMethod.class::cast).collect(Collectors.toList()))
//			FIXME realizacoes nao sao instanciadas no modelo (apenas visualmente)
				.addParameter(REALIZATIONS, class_.getInterfaceRealizations())
				.addParameter(GENERALIZATIONS, class_.getGeneralizations());
